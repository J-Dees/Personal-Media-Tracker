...
@router.post("user/{user_id}/social")
def add_follower():
    # add a follower by name, INSERT the user_id/follower_id pair into social table
    return "OK"

@router.get("user/{user_id}/social/search")
def search_follower():
    # searches through a user's followers based on query
    return "OK"

@router.delete("user/{user_id}/social/{follower_id}")
def remove_follower():
    # DELETE FROM social the user_id/follower_id pair
    return "OK"

@router.get("user/{user_id}/social/{follower_id}/catalogs")
def view_follower_catalogs():
    # view a specified follower's catalogs
    return "OK"

@router.get("user/{user_id}/social/{follower_id}/catalogs/{catalog}/recommendations")
def get_recommended():
...
@router.get("user/{user_id}/social/search/{title}")
def search_catalogs():
